define(function () {
  ...
  return function (container) {
    ...
    mouse.onDragOver = function (dragOverEvent) {
      dragOverEvent = dragOverEvent || function () {};
      mouse.dragOverEvents.push(dragOverEvent);
    };

    mouse.onDrop = function (dropEvent) {
      dropEvent = dropEvent || function () {};
      mouse.dropEvents.push(dropEvent);
    };
    ...
    mouse.updatePosition = function (newPosition) {
      ...
      mouse.previousPosition = mouse.position != null ? {
        x: mouse.position.x,
        y: mouse.position.y
      } : null;
      mouse.position = newPosition !== null ? {
        x: newPosition.x,
        y: newPosition.y
      } : null;
    ...
    /** Perform action for drag event */
    mouse.fireDragOverEvent = function (event) {
      for (var i = 0; i < mouse.dragOverEvents.length; i++) {
        mouse.dragOverEvents[i](event);
      }
    };

    /** Perform action for drop event */
    mouse.fireDropEvent = function (event) {
      for (var i = 0; i < mouse.dropEvents.length; i++) {
        mouse.dropEvents[i](event);
      }
    };
    ...
    mouse.moveEventMethod = function (event) {
      if (mouse.isTouching === false) {
        ...
        mouse.stoppingThread = setTimeout(function () {
          mouse.stopEventMethod(event);
        }, 50);
    ...
    mouse.dragOverEventMethod = function (event) {
      /* Put mouse as a reference in the event. */
      event.mouse = mouse;
      ...
      mouse.fireDragOverEvent(event);
    };

    /** When the mouse has dropped something in the container. */
    mouse.dropEventMethod = function (event) {
      /* Put mouse as a reference in the event. */
      event.mouse = mouse;
    ...
    mouse.touchStartMethod = function (event) {
      ...
      var offsetPosition = getOffsetPosition(mouse.listenElement);
      mouse.updatePosition({
        x: event.changedTouches[0].clientX - offsetPosition.x,
        y: event.changedTouches[0].clientY - offsetPosition.y
      });
    ...
    mouse.addDocumentMouseListener = function () {
      mouse.documentMouseListener = {
        mouse: mouse,
        moveEvent: mouse.moveEventMethod,
        upEvent: mouse.upEventMethod
      };
